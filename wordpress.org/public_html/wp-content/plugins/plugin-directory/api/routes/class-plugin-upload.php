...
class Plugin_Upload extends Base {
	...
	function __construct() {
		register_rest_route( 'plugins/v1', '/upload/(?P<ID>[0-9]+)', array(
			...
			'permission_callback' => function( $request ) {
				...
				if ( $post->ID != $request['ID'] || 'new' !== $post->post_status ) {
					return new WP_Error( 'cant_change_slug', __( "You can't change the slug of this plugin.", 'wporg-plugins' ) );
				}

				// Has it already been changed once?
				if ( $post->{'_wporg_plugin_original_slug'} && ! current_user_can( 'plugin_approve' ) ) {
					return new WP_Error( 'cant_change_slug', __( "You can't change the slug of this plugin.", 'wporg-plugins' ) );
				}
	...
	protected function perform_slug_change( $plugin, $slug ) {
		...
		// a. Add an audit log entry. This is important, such that the reviewer is aware of the change and need to review it.
		// b. Note the original slug, to prevent it being changed again.
		// c. Update the slug.
