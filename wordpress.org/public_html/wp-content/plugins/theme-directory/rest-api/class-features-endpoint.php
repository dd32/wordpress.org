...
class Features_Endpoint {
	...
	function features( $request ) {
		...
		if (
			// If WP_Version isn't set.
			! $wp_version &&
			// And it's an older API endpoint
			( ! defined( 'THEMES_API_VERSION' ) || THEMES_API_VERSION <= 1.1 ) &&
			// And it's a WordPress User Agent
			preg_match( '|WordPress/([^;]+)|', $request->get_header( 'user_agent' ), $matches )
		) {
		...
		if ( $wp_version && version_compare( $wp_version, '3.7.999', '<' ) ) {
		...
		if ( ! $wp_version || version_compare( $wp_version, '3.9-beta', '>' ) ) {
		...
		if ( ! $wp_version || version_compare( $wp_version, '4.6-alpha', '>' ) ) {
		...
		if ( ! $wp_version || version_compare( $wp_version, '5.2-alpha', '>=' ) ) {
		...
		if ( ! $wp_version || version_compare( $wp_version, '5.5-alpha', '>=' ) ) {
